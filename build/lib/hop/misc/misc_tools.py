...
def create_output_directories(output_folder, subfolders_to_be_made):
    ...
    folders = dict(
        zip(
            ...
            [
                Path(f"{output_folder}/{subfolder}")
                for subfolder in subfolders_to_be_made
            ],
        )
    )
...
def set_up_loggers(config):
    ...
    if not logger.handlers:
        ...
        file_handler = handlers.WatchedFileHandler(
            filename=f"{output_folder}/Logs/{output_filename_stem}_{current_time}.log"
        )
    ...
    if not logger_R_code.handlers:
        file_handler_R_code = handlers.WatchedFileHandler(
            filename=f"{output_folder}/Logs/{output_filename_stem}_{current_time}_R_Output.log"
        )
...
def update_header(filename, header_dictionary):
    ...
    with open(filename, "r+") as f:
        ...
        for key, value in reversed(header_dictionary.items()):
            print(f"{key},{value}")
