...
class Test_probe_functions:
    ...
    @given(magnet_center=st.tuples(st.floats(min_value=-1000, max_value=1000.0), st.floats(min_value=0.0, max_value=1000.0)))
    def test_circular_magnet_centre_orientation(self, mock_probe, magnet_center):

        mock_probe.circular_magnet_center = magnet_center
        ...
        true_value = np.degrees(np.arctan2(magnet_center[1], magnet_center[0]))
        if (magnet_center[1] == 0.0) & (magnet_center[0] == -0.0):
            true_value = 0.0
        print(f"Code value is {circular_magnet_orientation}, true value is {true_value}")
