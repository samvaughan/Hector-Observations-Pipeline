...
def overall_hexabundle_size_allocation_operation_version1(
    all_magnets, galaxyIDrecord, clusterNum, tileNum, flagsFile
):
    ...
    for hexabundleLetter in hexabundleDict:
        hexabundleDict[hexabundleLetter] = re.split(
            "(\d+)", hexabundleDict[hexabundleLetter]
        )[0:2]
    ...
    for i in all_magnets:
        ...
        for recordID in galaxyIDrecord:
            ...
            if (
                i.IDs == recordID
                and i.__class__.__name__ == "rectangular_magnet"
                and galaxyIDrecord[recordID] != "U"
                and galaxyIDrecord[recordID] != "H"
            ):
                ...
                if galaxyIDrecord[recordID] in hexabundleRepeatwithsameID:
                    ...
                    while case in hexabundleRepeatwithsameID:
                        ...
                        # All AAOmega hexabundles allocated, so shifting to Spector and recording as flag
                            ...
                            with open(flagsFile, "a") as fp:
                                writeToFile = (
                                    "ID CHECK. All AAOmega  hexabundles allocated, so shifting to Spector. Cluster/Batch :"
                                    + str(clusterNum)
                                    + ",tile "
                                    + str(tileNum)
                                    + "; "
                                    ...
                                    + str(recordID)
                                    + ", hexabundle: "
                                    + str(galaxyIDrecord[recordID])
                                    + "\n"
                                )
                                fp.write(writeToFile)
                            print(
                                "ALL AAOMEGA HAS BEEN ALLOCATED SO HECTOR WILL BE SELECTED NOW"
                            )
                        # All Spector hexabundles allocated, so shifting to AAOmega and recording as flag
                            ...
                            with open(flagsFile, "a") as fp:
                                writeToFile = (
                                    "ID CHECK. All Spector hexabundles allocated, so shifting to AAOmega. Cluster/Batch :"
                                    + str(clusterNum)
                                    + ",tile "
                                    + str(tileNum)
                                    + "; "
                                ...
                                fp.write(writeToFile)
                            print(
                                "ALL SPECTOR HAS BEEN ALLOCATED SO AAOMEGA WILL BE SELECTED NOW"
                            )
                        ...
                        # Move back to first Spector hexabundle in case of last one being in records already
                            ...
                            with open(flagsFile, "a") as fp:
                                writeToFile = (
                                    "EXCEPTIONAL CASE OF UNRESOLVED REPEAT !!! \n"
                                )
                                fp.write(writeToFile)
                            break
                    ...
                    hexabundleRepeatwithsameID.insert(
                        len(hexabundleRepeatwithsameID), case
                    )
                ...
                # Galaxy ID match not found in the records
                    ...
                    MagnetDict[i.__class__.__name__ + " " + str(int(i.index))][
                        "hexabundle"
                    ] = galaxyIDrecord[recordID]
                    hexabundleRepeatwithsameID.insert(
                        len(hexabundleRepeatwithsameID), galaxyIDrecord[recordID]
                    )
                    # print(hexabundleRepeatwithsameID)
        ...
        if (
            str(i.IDs) != "nan"
            and i.__class__.__name__ == "rectangular_magnet"
            and (i.IDs not in galaxyIDrecord)
        ):
    ...
    Re_Sort = sorted(
        MagnetDict.items(), key=lambda x: x[1]["Re"], reverse=True
    )  # largest Re at first
    for magnet in Re_Sort:
        ...
        if (
            ("rectangular_magnet" in magnet[0])
            and (magnet[1]["hexabundle"] == "NA")
            and (12 > magnet[1]["Re"] > 5)
            and hexabundleIndexAAOmega < 7
            and magnet[1]["mu_1re"] < 23
        ):
    ...
    hexabundleIndexThirdCriteria = (
        (HexaCount - 2) - hexabundleIndexAAOmega - hexabundleIndexHector
    )
    for magnet in Re_Sort:
        ...
        if (
            ...
            # removing the allocated hexabundle from the hexabundle dictionary
            hexabundleDict.pop(
                MagnetDict[magnet[0]]["hexabundle"], None
            )  # del hexabundleDict[MagnetDict[magnet[0]]['hexabundle']]
    ...
    for magnet in Mstar_Sort:
        ...
        if (
            ...
            for item in sorted(
                hexabundleDict.items(), key=lambda p: p[0][0], reverse=True
            ):
                # print(item[0])
    ...
    (
        ...
        hexabundleRepeatwithsameID,
        indexx,
    ) = resolve_hexabundle_allocation_repeats(
        result, hexabundleRepeatwithsameID, MagnetDict
    )
    ...
    MagnetDict = resolve_unallocated_hexabundle(
        unallocatedOnes, result, MagnetDict, indexx
    )

    # recording hexabundle as per galaxy records; IDs in current tile have been recorded above
    galaxyIDrecord, MagnetDict = update_galaxyIDrecord_withHexabundle(
        galaxyIDrecord, MagnetDict
    )

    # assigning allocated hexabundles to respective magnet as stored in Magnet Dictionary
    guideIndex = 1
    for magnet in all_magnets:
        magnet, guideIndex = hexabundle_allocation_fromMagnetDict(
            MagnetDict, magnet, guideIndex
        )

    # copying hexabundle titles from rectangular magnets to circular magnets
    for magnet in all_magnets:
        if (magnet.hexabundle == "NA") and (
            magnet.__class__.__name__ == "circular_magnet"
        ):
            for magnet2 in all_magnets:
                if (magnet.index == magnet2.index) and (
                    magnet2.__class__.__name__ == "rectangular_magnet"
                ):
...
def overall_hexabundle_size_allocation_operation_version3_largerBundlePriority(
    all_magnets, galaxyIDrecord, mu_1re_cutoff, clusterNum, flagsFile
):
    ...
    for i in all_magnets:
        ...
        if (
            i.__class__.__name__ == "rectangular_magnet"
            and i.galaxyORstar == 1
            and i.Re < 12
        ):
            answer = answer + [i.Re]
        elif (
    ...
    for magnet in Re_Sort:
        if (
            ("rectangular_magnet" in magnet[0])
            and (magnet[1]["galaxyOrStar"] == 1)
            and (magnet[1]["hexabundle"] == "NA")
            and (magnet[1]["Re"] < 12)
            and large_bundle_index < large_bundle_count
        ):
            for hexabundle in hexabundleDict:
                if (
                    int(hexabundleDict[hexabundle][1]) > 61
                ):  # check for larger hexabundles
                    MagnetDict[magnet[0]]["hexabundle"] = hexabundle
                    # print(magnet)
    ...
    for magnet in Mstar_Sort:  # Spector
        if (
            ...
            and (magnet[1]["galaxyOrStar"] == 1)
            and (magnet[1]["hexabundle"] == "NA")
            and (magnet[1]["mu_1re"] < X)
            and hexabundleIndexHector < 12
        ):
            for hexabundle in hexabundleDict:
                if (
                    int(hexabundleDict[hexabundle][1]) == 61
                    and hexabundleDict[hexabundle][0] == "S"
                ):  # check for larger hexabundles
                    MagnetDict[magnet[0]]["hexabundle"] = hexabundle
                    # print(magnet)
    ...
    for magnet in Mstar_Sort:  # AAOmega
        if (
            ...
            for hexabundle in hexabundleDict:
                if (
                    int(hexabundleDict[hexabundle][1]) == 61
                    and hexabundleDict[hexabundle][0] == "A"
                ):  # check for larger hexabundles
                    MagnetDict[magnet[0]]["hexabundle"] = hexabundle
                    # print(magnet)
...
def overall_hexabundle_size_allocation_operation_version2_median(
    all_magnets, galaxyIDrecord, clusterNum, tileNum, flagsFile
):
    ...
    for magnet in Mstar_Sort:
        if (
            ("rectangular_magnet" in magnet[0])
            and (magnet[1]["mu_1re"] < 22)
            and (magnet[1]["galaxyOrStar"] == 1)
            and (Mstar_Sort_count < lowMstar_spector_number)
        ):
            key = str(magnet[0])
            ...
            and (magnet[1]["mu_1re"] < 22)
            and (magnet[1]["galaxyOrStar"] == 1)
            and ((HexaCount - 2) > Mstar_Sort_count >= lowMstar_spector_number)
        ):
            key = str(magnet[0])
        ...
        if (
            ...
            for magnet_2nd in Mstar_median_lowerhalf:
                if str(magnet_2nd) == str(magnet[0]) and hexabundleIndexHector < 12:
                    ...
                    for hexabundle in hexabundleDict:
                        if (
                            hexabundleDict[hexabundle][0] == "S"
                        ):  # check for first Spector
                            MagnetDict[magnet[0]]["hexabundle"] = hexabundle
                            # print(magnet)
                            hexabundleIndexHector += 1
        ...
        if (
            ...
            for magnet_2nd in Mstar_median_higherhalf:
                if str(magnet_2nd) == str(magnet[0]) and hexabundleIndexAAOmega < 7:
                    ...
                    for hexabundle in hexabundleDict:
                        if (
                            hexabundleDict[hexabundle][0] == "A"
                        ):  # check for first AAOmega
                            MagnetDict[magnet[0]]["hexabundle"] = hexabundle
                            # print(magnet)
                            hexabundleIndexAAOmega += 1
...
def get_probe_count(MagnetDict):
    ...
    for magnet in MagnetDict:
        if "rectangular_magnet" in magnet:
            if (
                MagnetDict[magnet]["galaxyOrStar"] == 1
                or MagnetDict[magnet]["galaxyOrStar"] == 0
            ):
                HexaCount += 1
            else:
                GuidesCount += 1
...
def star_hexabundle_allocation(MagnetDict, hexabundleDict):
    ...
    for magnet in MagnetDict:
        if (
            ("rectangular_magnet" in magnet)
            and (MagnetDict[magnet]["galaxyOrStar"] == 0)
            and (MagnetDict[magnet]["hexabundle"] == "NA")
        ):
            for hexabundle in hexabundleDict:
...
def resolve_hexabundle_allocation_repeats(
    result, hexabundleRepeatwithsameID, MagnetDict
):
...
def resolve_unallocated_hexabundle(unallocatedOnes, result, MagnetDict, indexx):
    ...
    if unallocatedOnes != False:
        ...
        indexx += 1
        print(
            str(MagnetDict[unallocatedOnes]["hexabundle"])
            + "has been allocated as it was found unallocated."
        )
    else:
        ...
        if type(value) is dict:
            ...
            for key1, value1 in value.items():
                if key1 == "hexabundle":
                    if (
                        (value1 == "NA")
                        and ("rectangular_magnet" in key)
                        and (
                            nested_dictionary[key]["galaxyOrStar"] == 1
                            or nested_dictionary[key]["galaxyOrStar"] == 0
                        )
                    ):
...
def hexabundle_allocation_fromMagnetDict(MagnetDict, magnet, guideIndex):
    ...
    magnet.hexabundle = MagnetDict[
        magnet.__class__.__name__ + " " + str(int(magnet.index))
    ]["hexabundle"]

    # allocating hexabundle to guide probes in order GS1,2..
    if (magnet.hexabundle == "NA") and (
        magnet.__class__.__name__ == "rectangular_magnet"
    ):
