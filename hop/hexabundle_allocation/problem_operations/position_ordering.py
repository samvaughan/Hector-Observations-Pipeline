from .conflicts.functions import (
    create_list_of_blocking_magnets,
    all_blocking_magnets_are_fully_blocked,
)
from ..general_operations.trigonometry import convert_radians_to_degrees
from .hexabundle import (
    create_Magnet_Dictionary,
    get_probe_count,
    star_hexabundle_allocation,
    cores91_hexabundles_sortedByMstar,
    resolve_hexabundle_allocation_repeats,
    resolve_unallocated_hexabundle,
    check_for_hexabundle_allocation_repeats,
    check_for_unallocated_hexabundle,
    update_galaxyIDrecord_withHexabundle,
    hexabundle_allocation_fromMagnetDict,
    overall_hexabundle_size_allocation_operation_version1,
...
from ..problem_operations.robot_parameters import (
    assign_magnet_labels,
    add_pickupDirection,
    assign_preferable_pickupDirection,
    calculate_pickup_putdown_angles,
    check_for_negative_twopi_angle,
    check_for_unresolvable_conflict,
)
...
#
def calculate_placement_ordering_of_blocked_magnet(
    blocked_magnet, list_of_conflicts, list_of_fully_blocked_magnets
):
    blocking_magnets = create_list_of_blocking_magnets(
        list_of_conflicts, blocked_magnet
    )

    for i in range(50):
        if not all_blocking_magnets_are_fully_blocked(
            blocking_magnets, list_of_fully_blocked_magnets
        ):
            ...
            for magnet in blocking_magnets:
                blocking_magnets.append(
                    create_list_of_blocking_magnets(list_of_conflicts, magnet)
                )
    ...
    if i == 49:
        ...
        print(
            "Error ! ",
            blocked_magnet.__class__.__name__,
            int(blocked_magnet.index),
            " cannot be placed",
        )


# calculating placement ordering number of all the blocked magnets
def calculate_placement_ordering_of_all_blocked_magnets(
    list_of_fully_blocked_magnets, list_of_conflicts
):
    for blocked_magnet in list_of_fully_blocked_magnets:
        calculate_placement_ordering_of_blocked_magnet(
            blocked_magnet, list_of_conflicts, list_of_fully_blocked_magnets
        )


# creating position ordering array which is one of the main outputs of the whole magnet collision code
def create_position_ordering_array(
    all_magnets,
    ...
    calculate_placement_ordering_of_all_blocked_magnets(
        fully_blocked_magnets, conflicted_magnets
    )

    # maximum placement ordering number is derived
    max_order = max(
        magnet.placement_index
        for magnet in all_magnets
        if magnet.placement_index is not None
    )
    ...
    for magnet in all_magnets:
        ...
        order = check_for_unresolvable_conflict(
            magnet, conflictFile, max_order, clusterNum, tileName
        )
        ...
        magnet.rotation_putdown = check_for_negative_twopi_angle(
            magnet.rotation_putdown
        )
        ...
        f = np.append(
            [
                magnet.__class__.__name__,
                str(magnet.magnet_label),
                str(robot_center_x + magnet.view_x),
                str(robot_center_y + magnet.view_y),
                str(magnet.rotation_pickup),
                str(magnet.rotation_putdown),
                str(order),
                available_pickup,
                str(magnet.IDs),
                str(int(magnet.index)),
                str(magnet.hexabundle),
                str(probe_orientation),
            ],
            str(float(magnet.rectangular_magnet_input_orientation)),
        )
