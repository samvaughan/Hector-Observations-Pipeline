...
def load_table(config, quiet=False):
    ...
    assert (
        np.unique(columns_to_select).size == columns_to_select.size
    ...
    renamed_columns = dict(zip(final_table.columns.tolist(), column_names_for_renaming))
    ...
    if not quiet:
        ...
        for old_name, new_name in renamed_columns.items():
            ...
            if new_name in ["Mstar", "z", "Re", "r_mag", "i_mag"]:
...
def sparse_select_flat_in_Mass(random_generator, table, bins, MStarSparseCutBin):
    """
    ...
    USE A NUMPY RANDOM GENERATOR TO DO THIS! Otherwise this isn't repeatable
    and our catalogues get all messed up :-()
    """
    ...
    for bin_num in np.unique(bin_indices):
        ...
        if bin_num <= MStarSparseCutBin:
            ...
            if N_in_bin <= Num_per_Mstarbin:
                ...
                random_choice = random_generator.choice(
                    indices_in_bin, Num_per_Mstarbin, replace=False
                )
                indices_to_keep += random_choice.tolist()
...
def sparse_select_red_sequence(table, fraction_to_keep, random_generator):
    """Only select red-sequence galaxies from a table

    Args:
        table (pd.DataFrame): a pandas dataframe which we've been working on. Must have a column named "RS_member"
        fraction_to_keep (float): The fraction of red sequence galaxies to keep
        random_generator (np.random.Generator): A numpy random generator object

    Returns:
        _type_: _description_
    """

    # The fraction of galaxies to keep in each group
    # False refers to blue cloud galaxies, true to red sequence galaxies
    fraction_per_group_dict = {False: 1.0, True: fraction_to_keep}
    # group by red sequence membership, then sample each group
    table = table.copy().reset_index()
    sparsely_sampled = (
        table.groupby("RS_member")
        .apply(
            lambda group: group.sample(
                frac=fraction_per_group_dict[group.name], random_state=random_generator
            )
        )
        .reset_index(drop=True)
    )
    sparsely_sampled = sparsely_sampled.set_index("index")

    return sparsely_sampled
...
def add_boxes_to_Mstar_z(fig, ax, table_data):
    ...
    for i in [0, 1, 2, 3, 4]:
        for j in [0, 1, 2, 3]:
            ax.plot(
                [zdivisions[i], zdivisions[i + 1]],
                [Mstardivisions[j], Mstardivisions[j]],
                "-.",
                color="black",
            )
            ...
            Num_per_boxarray = table_data[
                np.logical_and.reduce(
                    (
                        table_data.z > zdivisions[i],
                        table_data.z < zdivisions[(i + 1)],
                        table_data.Mstar > Mstardivisions[j],
                        table_data.Mstar < Mstardivisions[(j + 1)],
                    )
                )
            ]
            ...
            Num_per_box_reGT1pt5 = np.shape(
                Num_per_boxtemp[(Num_per_boxtemp > Frac_Re_Cut)]
            )  # Not sure why this is two and not our cut from the command line?
            ...
            FracGTRe1pt5 = int(
                100 * (safe_div((float(Num_per_box_reGT1pt5[0])), (float(Num_per_box))))
            )
            ...
            zboxcentre = (zdivisions[(i + 1)] + zdivisions[(i)]) / 2.0
            Mboxcentre = (Mstardivisions[(j + 1)] + Mstardivisions[(j)]) / 2.0
            ax.text(
                zboxcentre,
                Mboxcentre,
                str(FracGTRe1pt5),
...
def calculate_SB_at_R(Sersic_index, Mu_e, multiple_of_Re):
    ...
    SB_at_R = Mu_e + 2.5 * (b / np.log(10)) * (
        (multiple_of_Re) ** (1.0 / Sersic_index) - 1
    )
...
class HectorSim:
    ...
    def __init__(
        ...
        Args:
            ...
            SparseFunction (dict): Method to go from our BLUE sample to our MAGENTA sample. Must be "FlatInMass" or "RedSequence". Must have a key "name", which must be either (1 or "FlatInMass") or (2 or RedSequence). Method 1 selects MAGENTA galaxies using the following method- all galaxies below MSparseCut1 are selected. For galaxies above MSparseCut1, we take a random sample in mass such that each bin contains the same number of galaxies as in the MSparseCut1 bin. Method 2 randomly subsamples cluster galaxies on the red sequence and includes all blue-cloud galaxies. The remaining keys are either "MSparseCut1", the bin at which we start our sparse sampling to flatten off the mass distribution, or "fraction_red_to_keep", the fraction of red galaxies we retain.
        ...
        if BoundaryType in ["taipan", "Taipan", "TAIPAN"]:
            ...
            if os.path.exists(f"{BoundaryType}"):
                ...
                raise NameError(
                    f"Bounday Type {BoundaryType} not understood or file does not exist"
                )

        if SparseFunction["name"] == 1:
            SparseFunction["name"] = "FlatInMass"
        elif SparseFunction["name"] == 2:
            SparseFunction["name"] = "RedSequence"

        # Save our input parameters as class attributes
        self.zlimit = zlimit
        self.MstarMin = MstarMin
        self.MstarMax = MstarMax
        self.SparseFunction = SparseFunction["name"]
        if self.SparseFunction == "FlatInMass":
            self.MSparseCut1 = SparseFunction["MSparseCut1"]
        elif self.SparseFunction == "RedSequence":
            self.fraction_red_to_keep = SparseFunction["fraction_red_to_keep"]
        self.minRe = minRe
        ...
        if not self.quiet:
            ...
            if self.SparseFunction == "FlatInMass":
                ...
                print(f"Red fraction to keep is {self.fraction_red_to_keep}")
            else:
                raise NameError(
                    f"SparseFunction must be one of 'FlatInMass' or 'RedSequence': currently it's {self.SparseFunction}"
                )
        ...
        self.HI_info = False
        if not np.all(np.isnan(self.entire_table["WALLABYflag"])) or np.all(
            np.isnan(self.entire_table["Dingoflag"])
        ):
            self.HI_info = True
        ...
        self.entire_table["g_m_i"] = (
            self.entire_table.loc[:, "g_mag"] - self.entire_table.loc[:, "i_mag"]
        )
        self.entire_table["IFU_diam_2Re"] = 4 * self.entire_table.loc[:, "Re"]

        # Work out surface brightness at 2 and 3 Re if we can
        if not np.all(np.isnan(self.entire_table.GAL_MU_E_R)) | np.all(
            np.isnan(self.entire_table.SersicIndex_r)
        ):
            self.entire_table["GAL_MU_R_at_2Re"] = calculate_SB_at_R(
                self.entire_table.SersicIndex_r, self.entire_table.GAL_MU_E_R, 2
            )
            self.entire_table["GAL_MU_R_at_3Re"] = calculate_SB_at_R(
                self.entire_table.SersicIndex_r, self.entire_table.GAL_MU_E_R, 3
            )
            good_data_mask = (
                good_data_mask
                & (self.entire_table["GAL_MU_R_at_2Re"] < 40.0)
                & (self.entire_table["GAL_MU_R_at_2Re"] > 0.0)
                & (self.entire_table["GAL_MU_R_at_3Re"] < 40.0)
                & (self.entire_table["GAL_MU_R_at_3Re"] > 0.0)
            )
        else:
        ...
        assert (
            self.entire_table.shape[0] > 0
        ), "Looks like our table doesn't have any rows after getting rid of bad values of M and R_e"
        if not self.quiet:
            print(
        ...
        if self.BoundaryType == "SAMI":
            ...
            self.mag_mask = self.entire_table.r_mag < self.maglimitcut

        elif self.BoundaryType == "Taipan":
            self.maglimitcut = 17.0
            maglimit = f"i<{self.maglimitcut}"
            self.mag_mask = self.entire_table.i_mag < self.maglimitcut
        ...
        if np.all(np.isfinite([Dec_Min, Dec_Max])):
            ...
            self.Dec_mask = (self.entire_table.DEC > self.Dec_Min) & (
                self.entire_table.DEC < self.Dec_Max
            )
            if not self.quiet:
                print(
                    f"Selecting a strip in DEC between {self.Dec_Min} and {self.Dec_Max}"
                )
                print(
                    f"\tIn this stripe, we have {self.entire_table.loc[self.Dec_mask].shape[0]} rows\n"
                )
        ...
        if np.all(np.isfinite([Dec_Min, Dec_Max])):
            self.possible_to_observe = self.entire_table.loc[
                self.Mstar_cut_qz
                & self.mag_mask
                & self.zcut
                & self.Re_cut
            ...
            self.selection_function = self.entire_table.loc[
                self.Mstar_cut_pz
                & self.mag_mask
                & self.zcut
                & self.Re_cut
            ...
            self.possible_to_observe = self.entire_table.loc[
                self.Mstar_cut_qz & self.mag_mask & self.zcut & self.Re_cut
            ]
            self.selection_function = self.entire_table.loc[
                self.Mstar_cut_pz & self.mag_mask & self.zcut & self.Re_cut
            ]

        nbins = int(np.ceil((self.MstarMax - self.MstarMin) * 5.0))
        ...
        if self.SparseFunction == "FlatInMass":
            ...
            self.selection_function_sparsely_sampled = sparse_select_flat_in_Mass(
                random_generator,
            ...
            self.selection_function_sparsely_sampled = sparse_select_red_sequence(
                self.selection_function,
                self.fraction_red_to_keep,
                random_generator=random_generator,
            )
        else:
            raise NameError(
                f"SparseFunction name not understood: currently it's {self.SparseFunction} but must be either FlatInMass or RedSequence"
            )
    ...
    def general_selection(self, z):
        if type(z) == float:
            if z < self.sf_config["lower_lim"]:
                y = (
                    self.sf_config["lower_gradient"] * z
    ...
    def plot_parent_sample(self, fig, ax):
        ...
        if not np.all(np.isnan(self.entire_table.GAL_MU_E_R)):
            fig, ax = self._Mstar_z_plot(
                self.entire_table,
                fig,
                ax,
                colour_by=self.entire_table.GAL_MU_E_R,
                cbar_label="Surface Brightness",
                vmin=20,
                vmax=24,
                lw=lw,
                s=s,
        ...
        if self.HI_info:
            HI_mask = (self.entire_table.WALLABYflag > 0) | (
                self.entire_table.Dingoflag > 0
            )
            ax.scatter(
                self.entire_table.z.loc[HI_mask],
    ...
    def plot_Mstar_z_gi_colour(self, fig, ax):
        ...
        if not np.all(np.isnan(self.entire_table.i_mag)):
            fig, ax = self._Mstar_z_plot(
                self.entire_table,
                fig,
                ax,
                colour_by=self.entire_table["g_m_i"],
                cbar_label="g - i colour",
                vmin=0,
                vmax=2,
            )
        else:
    ...
    def _Mstar_z_plot(
        self, table, fig, ax, colour_by=None, cbar_label=None, lw=0, s=1, **kwargs
    ):
        # FIXME Add in H1
        """
        ...
        if colour_by is None:
            ax.scatter(x=table.z, y=table.Mstar, marker=".", lw=lw, s=s, c="grey")
            ax.scatter(
    ...
    def plot_selected_sample_MZ(self, fig, ax):
        ...
        ax.scatter(
            self.entire_table.z, self.entire_table.Mstar, color="grey", marker="."
        )
        ax.scatter(
            self.possible_to_observe.z,
            self.possible_to_observe.Mstar,
            color="red",
            marker=".",
            lw=0,
            s=2,
        ...
        ax.scatter(
            self.selection_function.z,
            self.selection_function.Mstar,
            color="blue",
            marker="o",
            lw=0,
            s=7,
        ...
        ax.scatter(
            self.selection_function_sparsely_sampled.z,
            self.selection_function_sparsely_sampled.Mstar,
            color="magenta",
            marker="o",
            lw=0,
            s=7,
        ...
        if self.HI_info:
            HI_mask = (self.selection_function_sparsely_sampled.WALLABYflag > 0) | (
                self.selection_function_sparsely_sampled.Dingoflag > 0
            )
            ax.scatter(
                self.selection_function_sparsely_sampled.z.loc[HI_mask],
    ...
    def plot_selected_sample_MZ_colour_Re(self, fig, ax):
        ...
        sc = ax.scatter(
            self.selection_function_sparsely_sampled.z,
            self.selection_function_sparsely_sampled.Mstar,
            c=self.selection_function_sparsely_sampled.Re,
            marker=".",
            linewidths=0,
            s=2,
            **cmap_kwargs,
        ...
        ax.plot(
            [0.03, 0.045], [8.6, 8.6], "-", color="black"
        )  # plotting SAMI filler lower limits
        ax.plot(
            [0.045, 0.06], [9.4, 9.4], "-", color="black"
        )  # plotting SAMI filler lower limits
        ax.plot(
            [0.06, 0.095], [10.3, 10.3], "-", color="black"
        )  # plotting SAMI filler lower limits
        ax.plot(
            [0.095, 0.115], [10.9, 10.9], "-", color="black"
        )  # plotting SAMI filler lower limits
        ...
        if self.total_area > 0:
            density_in_selection = np.round(
                self.selection_function.shape[0] * np.pi / self.total_area, decimals=0
            )
            density_above_SB_limit = np.round(
                self.possible_to_observe.shape[0] * np.pi / self.total_area, decimals=0
            )
            density_in_sparse_sampled_selection = np.round(
                self.selection_function_sparsely_sampled.shape[0]
                * np.pi
                / self.total_area,
                decimals=0,
            )
    ...
    def plot_Re_hist(self, fig, ax):
        Remin = 0
        Remax = 15
        bins = np.linspace(Remin, Remax, int(np.ceil((Remax - Remin) * 4.0)))

        fig, ax = self._plot_hist("Re", bins, fig, ax)

        ax.text(
            0.7, 0.92, "median Re", color="black", fontsize=8, transform=ax.transAxes
        )  # was 13, 600
        ax.text(
            0.75,
            0.85,
            np.round(np.nanmedian(self.possible_to_observe.Re), decimals=2),
            color="red",
            transform=ax.transAxes,
        )
        ax.text(
            0.75,
            0.8,
            np.round(np.nanmedian(self.selection_function.Re), decimals=2),
            color="blue",
            transform=ax.transAxes,
        )
        ax.text(
            ...
            0.75,
            np.round(
                np.nanmedian(self.selection_function_sparsely_sampled.Re), decimals=2
            ),
            color="magenta",
            transform=ax.transAxes,
        )
    ...
    def plot_SB_Re(self, fig, ax):
        ...
        ax.plot(
            self.selection_function.Re,
            self.selection_function.GAL_MU_E_R,
            "o",
            color="blue",
        )
        ax.plot(
            self.selection_function_sparsely_sampled.Re,
            self.selection_function_sparsely_sampled.GAL_MU_E_R,
            ".",
            color="magenta",
        )
        ax.set_xlim(0, 15)
    ...
    def _get_fraction_of_sample_which_fits_in_IFU_diameter(self):
        ...
        pto_binned_IFU_diam, bin_edges_pto = np.histogram(
            self.possible_to_observe.IFU_diam_2Re, density=False, bins=200
        )

        N_sf = self.selection_function.IFU_diam_2Re.size
        sf_binned_IFU_diam, bin_edges_sf = np.histogram(
            self.selection_function.IFU_diam_2Re, density=False, bins=200
        )

        N_sfss = self.selection_function_sparsely_sampled.IFU_diam_2Re.size
        sfss_binned_IFU_diam, bin_edges_sfss = np.histogram(
            self.selection_function_sparsely_sampled.IFU_diam_2Re,
            density=False,
            bins=200,
        )

        fractions = [
            pto_binned_IFU_diam / N_pto,
            sf_binned_IFU_diam / N_sf,
            sfss_binned_IFU_diam / N_sfss,
        ]
    ...
    def cumulative_fraction_sample_imaged_to_2Re(self, fig, ax):
        ...
        )  # assumes 105um cores with 5um cladding.
        # coresize = 105
        CORES = np.array(
            [
                1,
        ...
        for i in range(0, MAX_bundle):
            ...
            bundles = (
                bisect.bisect(bin_edges_sf[:-1], DIA[i]) - 1
            )  # find first bin in the cumulative bundle diameter for 2Re plot
        ...
        bundles_frac_magenta = np.hstack(
            bundles_frac_magenta
    ...
    def plot_SB_hist(self, fig, ax):
        ...
        if not np.all(np.isnan(self.possible_to_observe.GAL_MU_E_R)):
            ...
            if not np.all(np.isnan(self.possible_to_observe.GAL_MU_R_at_2Re)):
                ax.hist(
                    self.possible_to_observe.GAL_MU_R_at_3Re,
                    color="red",
                    bins=bins,
                    alpha=0.2,
                )
                ax.hist(
                    self.selection_function.GAL_MU_R_at_3Re,
                    color="blue",
                    bins=bins,
                    alpha=0.2,
                )
                ax.hist(
                    self.selection_function_sparsely_sampled.GAL_MU_R_at_3Re,
                    color="magenta",
                    bins=bins,
                    alpha=0.2,
                )
                ...
                ax.hist(
                    self.possible_to_observe.GAL_MU_R_at_2Re,
                    color="red",
                    bins=bins,
                    alpha=0.5,
                )
                ax.hist(
                    self.selection_function.GAL_MU_R_at_2Re,
                    color="blue",
                    bins=bins,
                    alpha=0.5,
                )
                ax.hist(
                    self.selection_function_sparsely_sampled.GAL_MU_R_at_2Re,
                    color="magenta",
                    bins=bins,
                    alpha=0.5,
                )
    ...
    def plot_percentage_galaxy_sample_imaged_2Re(self, fig, ax):
        ...
        (
            percent100umcores,
            percent100umcores_zLT0pt1,
            percent75umcores,
            cores9,
            cores13,
            TotalFracto2Re,
        ) = self._get_percent_100um_cores()

        # ax = fig3.add_subplot(2,4,8)
        ax.plot(
            cores9, percent100umcores[:, 1], "-o", color="magenta", label="105um fibres"
        )
        ax.plot(
            cores9,
            percent100umcores_zLT0pt1[:, 1],
            "-o",
            color="blue",
            label="105um fibres",
        )
        ...
        ax.text(
            50,
            3,
            "Percentage (magenta)\nimaged to 2Re = " + str(TotalFracto2Re) + "%",
            color="magenta",
            size=8,
            ha="left",
            bbox=dict(
    ...
    def _get_percent_100um_cores(self):
        ...
        )  # Note:points at 28 are for everything >25.897
        percent100umcores_zLT0pt1 = np.array(
            [
                [1.726476, 1],
                [5.17942857, 1],
                [8.63238, 1],
        ...
        )  # Note:points at 30 are for everything >28.5
        percent75umcores = np.array(
            [
                [1.219047619, 1],
                [3.657142857, 1],
                [6.095238095, 1],
        ...
        )  # Note:points at 35 are for everything >30.5
        lenpc100 = len(percent100umcores)
        lenpc75 = len(percent75umcores)

        percentfinalbundledistribution = np.array(
            [
                [1.726476, 0],
                [5.17942857, 0],
                [8.63238, 0],
                [12.085333, 0],
                [15.5382857, (14.0 / 19.0) * 100],
                [18.99123809, (2.0 / 19.0) * 100],
                [22.44419, (1.0 / 19.0) * 100],
                [25.89714, (2.0 / 19.0) * 100],
                [28, 0.0],
            ]
        )  # excluding the 2 star bundles

        for i in range(lenpc100):
            if i == 0:
                test1 = float(
                    len(bundlediam_2Re[(bundlediam_2Re < percent100umcores[i, 0])])
                )
                test3 = ((test1) / (float(len(bundlediam_2Re)))) * 100
                percent100umcores[i, 1] = test3
                test1 = float(
                    len(
                        bundlediam_2Re_zLT0pt1[
                            (bundlediam_2Re_zLT0pt1 < percent100umcores[i, 0])
                        ]
                    )
                )
            ...
            if i > 0:
                ...
                test2 = float(
                    len(bundlediam_2Re[(bundlediam_2Re < percent100umcores[i - 1, 0])])
                )
                ...
                test2 = float(
                    len(
                        bundlediam_2Re_zLT0pt1[
                            (bundlediam_2Re_zLT0pt1 < percent100umcores[i - 1, 0])
                        ]
                    )
                )
        ...
        for i in range(lenpc75):
            if i == 0:
                test1 = float(
                    len(bundlediam_2Re[(bundlediam_2Re < percent75umcores[i, 0])])
                )
                test3 = ((test1) / (float(len(bundlediam_2Re)))) * 100
            ...
            if i > 0:
                ...
                test2 = float(
                    len(bundlediam_2Re[(bundlediam_2Re < percent75umcores[i - 1, 0])])
                )
                test3 = ((test1 - test2) / (float(len(bundlediam_2Re)))) * 100
                percent75umcores[i, 1] = test3
            if i == (lenpc75 - 1):
        ...
        if percent100umcores[8, 1] > percentfinalbundledistribution[7, 1]:
            numfit169 = 0
            numleft169 = (
                percent100umcores[8, 1]
                - percentfinalbundledistribution[7, 1]
                + percent100umcores[7, 1]
            )
        else:
            numfit169 = percentfinalbundledistribution[7, 1] - percent100umcores[8, 1]
            numleft169 = (
                percent100umcores[7, 1] - numfit169
            )  # number in the 169 bin that did not fit
        if numleft169 > percentfinalbundledistribution[6, 1]:
            numfit127 = 0
            numleft127 = (
                numleft169
                - percentfinalbundledistribution[6, 1]
                + percent100umcores[6, 1]
            )
        else:
        ...
        if numleft127 > percentfinalbundledistribution[5, 1]:
            ...
            numleft91 = (
                numleft127
                - percentfinalbundledistribution[5, 1]
                + percent100umcores[5, 1]
            )
        else:
        ...
        if numleft91 > percentfinalbundledistribution[4, 1]:
            ...
            numleft61 = (
                numleft91
                - percentfinalbundledistribution[4, 1]
                + percent100umcores[4, 1]
            )
        else:
            numfit61 = percentfinalbundledistribution[4, 1] - numleft91
            numleft61 = numleft91 - numfit61 + percent100umcores[4, 1]
        TotalFracto2Re = int(numfit169 + numfit127 + numfit91 + numfit61)
    ...
    def cumulative_percentage_galaxies_imaged_2Re(self, fig, ax):
        ...
        percent100umcores[4, 1] = (
            percent100umcores[0, 1]
            + percent100umcores[1, 1]
            + percent100umcores[2, 1]
            + percent100umcores[3, 1]
            + percent100umcores[4, 1]
        )
        ...
        percent75umcores[6, 1] = (
            percent75umcores[0, 1]
            + percent75umcores[1, 1]
            + percent75umcores[2, 1]
            + percent75umcores[3, 1]
            + percent75umcores[4, 1]
            + percent75umcores[5, 1]
            + percent75umcores[6, 1]
        )
        ...
        totalbundles = np.array(
            [100, 90, 80, 70, 60, 50, 20]
        )  # total number of hexabundles
        cores9_sumfibrenumbers_100 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[0]
        )
        cumcores9_sumfibrenumbers_100 = np.cumsum(cores9_sumfibrenumbers_100)
        cores9_sumfibrenumbers_90 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[1]
        )
        cumcores9_sumfibrenumbers_90 = np.cumsum(cores9_sumfibrenumbers_90)
        cores9_sumfibrenumbers_80 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[2]
        )
        cumcores9_sumfibrenumbers_80 = np.cumsum(cores9_sumfibrenumbers_80)
        cores9_sumfibrenumbers_70 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[3]
        )
        cumcores9_sumfibrenumbers_70 = np.cumsum(cores9_sumfibrenumbers_70)
        cores9_sumfibrenumbers_60 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[4]
        )
        cumcores9_sumfibrenumbers_60 = np.cumsum(cores9_sumfibrenumbers_60)
        cores9_sumfibrenumbers_50 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[5]
        )
        cumcores9_sumfibrenumbers_50 = np.cumsum(cores9_sumfibrenumbers_50)
        cores9_sumfibrenumbers_20 = (
            cores9 * (percent100umcores[:, 1] / 100) * totalbundles[6]
        )
        cumcores9_sumfibrenumbers_20 = np.cumsum(cores9_sumfibrenumbers_20)
        percentintervals = np.array(
            [
                0,
                5,
                10,
        ...
        percentintervals_100um_50 = np.interp(
            percentintervals, cum_pc100umcores, cumcores9_sumfibrenumbers_50
        )  # print the cumulative number of cores at 90% to work out total angular area. Note had to reverse x and y
        percentintervals_100um_100 = np.interp(
            percentintervals, cum_pc100umcores, cumcores9_sumfibrenumbers_100
        )
        ...
        cores13_sumfibrenumbers_100 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[0]
        )
        cumcores13_sumfibrenumbers_100 = np.cumsum(cores13_sumfibrenumbers_100)
        cores13_sumfibrenumbers_90 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[1]
        )
        cumcores13_sumfibrenumbers_90 = np.cumsum(cores13_sumfibrenumbers_90)
        cores13_sumfibrenumbers_80 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[2]
        )
        cumcores13_sumfibrenumbers_80 = np.cumsum(cores13_sumfibrenumbers_80)
        cores13_sumfibrenumbers_70 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[3]
        )
        cumcores13_sumfibrenumbers_70 = np.cumsum(cores13_sumfibrenumbers_70)
        cores13_sumfibrenumbers_60 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[4]
        )
        cumcores13_sumfibrenumbers_60 = np.cumsum(cores13_sumfibrenumbers_60)
        cores13_sumfibrenumbers_50 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[5]
        )
        cumcores13_sumfibrenumbers_50 = np.cumsum(cores13_sumfibrenumbers_50)
        cores13_sumfibrenumbers_20 = (
            cores13 * (percent75umcores[:, 1] / 100) * totalbundles[5]
        )
        ...
        percentintervals_75um_50 = np.interp(
            percentintervals, cum_pc75umcores, cumcores13_sumfibrenumbers_50
        )  # print the cumulative number of cores at 90% to work out total angular area. Note had to reverse x and y
        percentintervals_75um_100 = np.interp(
            percentintervals, cum_pc75umcores, cumcores13_sumfibrenumbers_100
        )
    ...
    def make_Julias_plot(self, fig, axs):
        ...
        if axs.shape != (3, 6):
            raise ValueError(
                f"Must pass axes of shape (3, 6)- these have shape {axs.shape}"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Simulate Hector Observations using a variety of input catalogues"
    )
    parser.add_argument(
        "BoundaryType", help="Must be Taipan or SAMI (not case sensitive)"
    )
    ...
    parser.add_argument(
        "SparseFunction",
        type=int,
        help="Sparse selection function. Must be 1 or 2 (FIXME)",
    )
    parser.add_argument("SourceCat", type=str, help="Short name for input catalogue")
    parser.add_argument(
        "MSparseCut1", type=float, help="Select flat in M* above this mass"
    )
    parser.add_argument(
        "minRe", type=float, help="Minimum Re of galaxies to select (in kpc? arcsec?)"
    )
    parser.add_argument(
        "--random-seed",
        type=int,
        help="A random seed for repeatable results",
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Suppress printing to the terminal"
    )
    parser.add_argument(
        "--Dec_Min",
        type=float,
        default=np.nan,
        help="Minimum Dec of the observations. Useful for selecting a strip of targets",
    )
    parser.add_argument(
        "--Dec_Max",
        type=float,
        default=np.nan,
        help="Maximum Dec of the observations. Useful for selecting a strip of targets",
    )

    args = parser.parse_args()

    config = _load_config(args.SourceCat)
    full_catalogue_table = load_table(config, quiet=args.quiet)
    HectorSim_args = vars(args)

    full_catalogue_table["PRI_SAMI"] = 8
    full_catalogue_table["remaining_observations"] = 1
    low_mass_galaxies_for_repeats = (full_catalogue_table["Mstar"] < 9) & (
        full_catalogue_table["Mstar"] > 0
    )
    full_catalogue_table.loc[
        low_mass_galaxies_for_repeats, "remaining_observations"
    ] = 3
    ...
    ax_skyplot.scatter(
        ...
        s=1,
        alpha=0.5,
        label="Entire Input Catalogue",
    )
    ax_skyplot.scatter(
        HS.possible_to_observe.RA,
        HS.possible_to_observe.DEC,
        c="red",
        s=1,
        alpha=0.5,
        label="Possible to observe (e.g. big/bright enough)",
    )
    ax_skyplot.scatter(
        HS.selection_function.RA,
        HS.selection_function.DEC,
        c="blue",
        s=1,
        alpha=0.5,
        label="Above selection function",
    )
    ax_skyplot.scatter(
        HS.selection_function_sparsely_sampled.RA,
        HS.selection_function_sparsely_sampled.DEC,
        c="magenta",
        s=2,
        label="Final catalogue (sparse sampling)",
    )
    plt.show()
    ...
    N_hexabundles = 19
    final_source_density = (
        N_galaxy_targets / HS.total_area
    ...
    print(
        f"Source Density in final catalogue (magenta points): {source_density_per_Hector_field:.1f} galaxies per 2DF field"
    )

    ## Print the fraction of galaxies below z=0.06
    fraction_below_0p6 = (HS.selection_function_sparsely_sampled.z < 0.06).sum() / len(
        HS.selection_function_sparsely_sampled
    )
