...
import pandas as pd
from ..general_operations.trigonometry import (
    rotational_matrix,
...
def annotate_text_in_plot(magnet, ax):
    rotation_matrix = rotational_matrix(
        convert_degrees_to_radians(magnet.plot_orientation)
    )
    ...
    magnet.corner = (
        magnet.center[0]
        + rotation_matrix[0][0] * (magnet.width + corner_offset) / 2
        + rotation_matrix[1][0] * (magnet.length + 3) / 2,
        magnet.center[1]
        + rotation_matrix[0][1] * (magnet.width + corner_offset) / 2
        + rotation_matrix[1][1] * (magnet.length + 3) / 2,
    )
    magnet.circular_center = (
        magnet.center[0] - rotation_matrix[1][0] * (magnet.length + 27) / 2,
    ...
    draw_fill2 = patches.Rectangle(
        (magnet.corner[0], magnet.corner[1]),
        width,
        (length - 12),
        angle=abs(180 - magnet.plot_orientation),
    ...
    draw_fill3 = patches.Circle(
        (magnet.circular_center[0], magnet.circular_center[1]),
        radius=radius,
        facecolor=patch_color,
        edgecolor="black",
        linewidth=0.5,
    ...
    magnet.text_center = (
        magnet.center[0] - rotation_matrix[1][0] * (magnet.length) / 2,
    ...
    if (magnet.hexabundle == "H") or (magnet.hexabundle == "U"):
        ...
        ax.annotate(
            annotated_star,
            (magnet.text_center[0], magnet.text_center[1]),
            color=label_color,
            rotation=abs(90 - magnet.plot_orientation),
            weight="bold",
            fontsize=11,
            ha="center",
            va="center",
...
def sky_fibre_annotations(fig, ax, skyfibre_file):
    ...
    ax = plot_skyfibre_section(
        fig,
        ax,
        skyfibreDict,
        angle=30,
        radius=radius,
        angle_subplate=angle_subplate,
        skyfibre_titles=skyfibreTitles_top,
        delta_ang=20,
    )
    ax = plot_skyfibre_section(
        fig,
        ...
        radius=radius,
        angle_subplate=angle_subplate,
        skyfibre_titles=skyfibreTitles_left,
        delta_ang=20,
    )
    ax = plot_skyfibre_section(
        fig,
        ...
        radius=radius,
        angle_subplate=angle_subplate,
        skyfibre_titles=skyfibreTitles_right,
        delta_ang=-20,
    )
...
def draw_circular_magnet(magnet, ax):
    ...
    circular_magnet = patches.Circle(
        (magnet.center[0], magnet.center[1]),
        radius=propdict["radius"],
        facecolor=propdict["patch_color"],
        edgecolor="k",
        linewidth=0.5,
        alpha=1,
        zorder=propdict["z_order"] + 1,
    )
    ax.add_patch(circular_magnet)
    ax.annotate(
        text=text,
        xy=(magnet.center[0], magnet.center[1]),
        color=propdict["label_color"],
        rotation=text_angle,
        weight="bold",
        fontsize=6,
        ha="center",
...
def draw_rectangular_magnet(magnet, ax):
    ...
    magnet_corner = (
        centre[0] - (propdict["width"]) / 2,
        centre[1] - (propdict["length"] - 12) / 2,
    )

    # Rotate the magnet around their centres
    transform = (
        Affine2D().rotate_deg_around(*(magnet.center), 180 - magnet.plot_orientation)
        + ax.transData
    )
    # Increase zorder of the smaller hexabundles over A and B and C- the largest ones
    # ****** previousOne draw_fill = patches.Rectangle((magnet.corner[0], magnet.corner[1]), width, length, angle=abs(180 - magnet.plot_orientation),facecolor=patch_color,alpha=0.8,zorder=3)#,edgecolor='black',linewidth=0.5
...
def draw_all_magnets(
    ...
    ax_hexa = sky_fibre_annotations(
        fig=fig_hexa, ax=ax_hexa, skyfibre_file=fileNameHexa
    )
    ...
    for hexabundle in all_hexabundles:
        circular_magnet_dict[hexabundle].angs = np.degrees(
            rectangular_magnet_dict[hexabundle].angs
        )
    ...
    for magnet in magnets:
        if is_circular_magnet(magnet):
            ...
            ax_robot.text(
                robot_center_x + magnet.center[0],
                robot_center_y + magnet.center[1],
                str(magnet.magnet_label),
                fontsize=8,
                rotation=abs(180 - magnet.plot_orientation),
                weight="bold",
