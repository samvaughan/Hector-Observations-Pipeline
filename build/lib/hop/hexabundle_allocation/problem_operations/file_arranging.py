...
def arrange_guidesFile(fileNameHexa, fileNameGuides, guide_outputFile):
    ...
    hexaCount = (
        df_hexas.probe.max()
    )  # int(df_hexas.loc[df_hexas.index[-1], 'probe']) + 1
...
def merge_hexaAndGuides(fileNameHexa, df_guideFile, plate_file):
    ...
    df_combined.to_csv(
        plate_file, index=False, sep=",", quoting=csv.QUOTE_NONE, escapechar=" "
    )
...
def create_robotFileArray(
    ...
    positioning_array[:, 8] = [
        i if i != "nan" else -999999 for i in positioning_array[:, 8]
    ]
    for i in range(len(positioning_array[:, 8])):
        # Add the Hexabundle ID to the robot file
        # We've recently changed our IDs to now include a letter at the start (W for WAVES galaxies, C for clusters)
        # It's not clear to me that the robot can deal with having strings in the ID column
        # I'm therefore getting rid of this starting number _only for the robot file_
        # The tile file will have the "correct" IDs.
        try:
            id_value = round(float(positioning_array[:, 8][i]))
        except ValueError:
            id_value = round(float(positioning_array[:, 8][i][1:]))
        positioning_array[:, 8][i] = id_value
    ...
    robotFilearray = add_repositionCol_to_robotFile(
        positioning_array, robotFilearray, fully_blocked_magnets_dictionary
    )
    ...
    with open(robotFile, "w+") as robotFile:
        ...
        robotFile.write(
            f"#Date_and_Time_file_created, {str(datetime.datetime.now().strftime('%d-%B-%y %H:%M:%S'))}\n"
        )

        robotFile.write(
            "#Radial_Offset_Adjustment, -9999 #Radial offset is in millimetre(mm) with +ve values actioning radial outward movement and -ve values actioning radial inward movement of the magnets. \n"
        )

        robotFile.write(
            f"#RobotTemp, {robot_temp:.4f} #temperature (degrees C) the distortion code assumed the field would be observed at\n"
        )
        robotFile.write(
            f"#ObsTemp, {obs_temp:.4f} #actual temperature (degrees C) it is going to be observed at\n"
        )

        robotFile.write(
            "#Radial_Scale_factor, 1.0 #Radial scale factor is thermal coefficient of invar times temperature difference applied radially relative to the plate centre.\n"
        )
        robotFile.write("\n\n")
...
def add_repositionCol_to_robotFile(
    positioning_array, robotFilearray, fully_blocked_magnets_dictionary
):
    ...
    robotFilearray = np.hstack(
        (robotFilearray[:, :8], nameColumn, robotFilearray[:, 8:])
    )
            ...
            if (robotFilearray[i][0] + " " + str(robotFilearray[i][10])) == each_magnet:
                for j in range(len(fully_blocked_magnets_dictionary[each_magnet])):
                    ...
                    for k in range(len(robotFilearray)):
                        if (
                            robotFilearray[k][0] + " " + str(robotFilearray[k][10])
                            == fully_blocked_magnets_dictionary[each_magnet][j]
                        ):
    ...
    # add the 'list' column to the robot file array in desired position
    robotFilearray = np.hstack(
        (robotFilearray[:, :9], rePosition_col, robotFilearray[:, 9:])
    )
...
def positioningArray_adjust_and_mergetoFile(
    ...
    positioning_array_circular = np.insert(
        positioning_array_circular, 0, np.array(newrow_circular), 0
    )
    positioning_array_circular = np.delete(
        positioning_array_circular, [2, 3, 4, 5, 8, 9, 10, 11, 12], 1
    )
    ...
    with open(plate_file, "r") as read_obj, open(
