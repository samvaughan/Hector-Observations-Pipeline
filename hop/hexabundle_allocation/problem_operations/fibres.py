...
def extract_fibreInfo(fibre_file,all_magnets,robotFilearray,output_fibreAAOmega,output_fibreSpector):
    ...
    new_arrayAAOmega[0] = new_arraySpector[0] = ['slit_pos', 'type(S/P)', 'probe_num', 'probe_fibre', \
                          'probe_orientation', 'Good/bad', 'label', 'slitlet', 'cores_count', 'ring', 'position_on_slit']
    ...
    for i in fibre_data['Hexabundle or sky fibre']:
        if fibre_data['Spectrograph'][0] == 'AAOmega' and j < 820:
            ...
            if str(i) == 'Sky':
                ...
                new_arrayAAOmega[j][5] = 'nan'
            elif str(i) == 'H':
                new_arrayAAOmega[j][1] = 'P'
                new_arrayAAOmega[j][5] = 'Good'
            elif str(i) == 'BLOCK':
                new_arrayAAOmega[j][5] = 'nan'
            new_arrayAAOmega[j][2] = new_arrayAAOmega[j][6] = fibre_data['Bundle/plate'][j-1]
            ...
            if fibre_data['no.cores'][j-1] > 0:
                new_arrayAAOmega[j][8] = round(fibre_data['no.cores'][j-1])
            else:
                new_arrayAAOmega[j][8] = 'nan'
            new_arrayAAOmega[j][7] = fibre_data['slitlet'][j-1]
            new_arrayAAOmega[j][9] = fibre_data['ring'][j-1]
            new_arrayAAOmega[j][10] = fibre_data['position on slit '][j-1]  #*** Might need to swap places with fibre number **
            ...
            if str(i) == 'Sky':
                ...
                new_arraySpector[j-819][5] = 'nan'
            elif str(i) == 'H':
                new_arraySpector[j-819][1] = 'P'
                new_arraySpector[j-819][5] = 'Good'
            elif str(i) == 'BLOCK':
                new_arraySpector[j-819][5] = 'nan'
            new_arraySpector[j-819][2] = new_arraySpector[j-819][6] = fibre_data['Bundle/plate'][j - 1]
            if fibre_data['Fibre_number'][j - 1] > 0:
                new_arraySpector[j-819][3] = round(fibre_data['Fibre_number'][j - 1])
            else:
                new_arraySpector[j-819][3] = 'nan'
            if fibre_data['no.cores'][j - 1] > 0:
                new_arraySpector[j-819][8] = round(fibre_data['no.cores'][j - 1])
            else:
                new_arraySpector[j-819][8] = 'nan'
            new_arraySpector[j-819][7] = fibre_data['slitlet'][j - 1]
            new_arraySpector[j-819][9] = fibre_data['ring'][j - 1]
            new_arraySpector[j-819][10] = fibre_data['position on slit '][j-1]  # *** Might need to swap places with fibre number **
