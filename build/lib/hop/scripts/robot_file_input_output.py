...
def read_parking_positions_file(filename):
    ...
    try:
        rectangular_magnets_start_index = all_lines.index(
            "Rectangular Magnets, , , , \n"
        )
        ...
        raise ValueError(
            "This file is not in the expected format- can't find where each magnet type starts. Exiting"
        )

    rectangular_magnets = all_lines[
        rectangular_magnets_start_index + 1 : blue_magnets_start_index
    ]
    blue_magnets = all_lines[blue_magnets_start_index + 1 : green_magnets_start_index]
    green_magnets = all_lines[
        green_magnets_start_index + 1 : yellow_magnets_start_index
    ]
    yellow_magnets = all_lines[
        yellow_magnets_start_index + 1 : magneta_magnets_start_index
    ]
    ...
    for data, name in zip(
        ...
        magnet_df = pd.DataFrame(
            [l.rstrip().split(",") for l in data],
            columns=["MagnetNumber", "Center_x", "Center_y", "z", "zero"],
        )
...
def write_standard_parking_positions_file(df, output_file):
    ...
    with open(output_file, "w") as f:
        ...
        df.loc[df.MagnetName == "RectangularMagnet"].to_csv(
            f,
            index=False,
            sep=",",
            mode="a",
        ...
        df.loc[df.MagnetName == "BlueCircularMagnet"].to_csv(
            f,
            index=False,
            sep=",",
            mode="a",
        ...
        df.loc[df.MagnetName == "GreenCircularMagnet"].to_csv(
            f,
            index=False,
            sep=",",
            mode="a",
        ...
        df.loc[df.MagnetName == "YellowCircularMagnet"].to_csv(
            f,
            index=False,
            sep=",",
            mode="a",
        ...
        df.loc[df.MagnetName == "MagnetaCircularMagnet"].to_csv(
            f,
            index=False,
            sep=",",
            mode="a",
