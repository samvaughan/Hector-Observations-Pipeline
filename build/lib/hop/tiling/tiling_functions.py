...
def get_best_tile_centre_greedy(
    targets_df, outer_FOV_radius, inner_FoV_radius, priorities, n_xx_yy=100
):
    """
    ...
    for i, coord in enumerate(grid_coords):
        galaxies_in_FoV[i, :] = check_if_in_fov(
            targets_df,
            grid_coords[i, 0],
            grid_coords[i, 1],
            inner_FoV_radius,
    ...
    n_targets_in_FOV = (galaxies_in_FoV * np.array(priorities)[None, :]).sum(
        axis=1
    )  #  - (galaxies_in_inner_FoV * np.array(priorities)[None, :]).sum(axis=1)
    ...
    if len(possible_tile_centres) > 1:
        index = _decide_upon_best_tile_centre(
            n_targets_in_FOV,
            possible_tile_centres,
            grid_coords,
            targets_in_best_tiles_mask,
            RA,
            Dec,
        )
    else:
...
def _decide_upon_best_tile_centre(
    n_targets_in_FOV,
    possible_tile_centres,
    grid_coords,
    targets_in_best_tiles_mask,
    ...
    for i, (centre, targets_mask) in enumerate(
        zip(possible_tile_centres, targets_in_best_tiles_mask)
    ):
        distances_from_centre[i] = np.sum(
            np.sqrt(
                (RA[targets_mask] - grid_coords[centre, 0]) ** 2
                + (Dec[targets_mask] - grid_coords[centre, 1]) ** 2
            ),
            axis=0,
        )
    ...
    weights = 1.0 / distances_from_centre
    norm_weights = weights / weights.sum()
    index = np.random.choice(
        np.where(n_targets_in_FOV == np.max(n_targets_in_FOV))[0], p=norm_weights
    )

    return index


def get_best_tile_centre_dengreedy(
    master_df, targets_df, outer_FOV_radius, inner_FoV_radius, n_xx_yy=100
):
    """
    ...
    for i, coord in enumerate(grid_coords):
        galaxies_in_FoV_all[i, :] = check_if_in_fov(
            master_df,
            grid_coords[i, 0],
            grid_coords[i, 1],
            inner_FoV_radius,
    ...
    for i, coord in enumerate(grid_coords):
        galaxies_in_FoV_untiled[i, :] = check_if_in_fov(
            targets_df,
            grid_coords[i, 0],
            grid_coords[i, 1],
            inner_FoV_radius,
    ...
    if np.sum(ratio_of_targets == np.max(ratio_of_targets)):
        index = np.random.choice(
            np.where(ratio_of_targets == np.max(ratio_of_targets))[0]
        )
    else:
...
def find_nearest(x, y, grid):
    ...
    if len(x) != len(y):
        raise ValueError(
            f"x and y must be the same length: Currently {len(x)} and {len(y)}"
        )
    if grid.ndim != 2:
    ...
    for i, (target_x, target_y) in enumerate(zip(x, y)):
        nearest_points[i] = np.argmin(
            np.sqrt((grid[:, 0] - target_x) ** 2 + (grid[:, 1] - target_y) ** 2)
        )
...
def check_if_in_fov(df, xcen, ycen, inner_radius, outer_radius):
    ...
    if inner_radius == 0:
        return (
            np.sqrt((x - xcen) ** 2 * cos_dec_correction**2 + (y - ycen) ** 2)
            < outer_radius
        ).values
    else:
        return (
...
def find_great_circle_distance(uu, vv):
    ...
    if np.all(uu == vv):
        ...
        theta = np.arccos(
            np.sin(uu[1]) * np.sin(vv[1])
            + np.cos(uu[1]) * np.cos(vv[1]) * np.cos(uu[0] - vv[0])
        )
...
def select_stars_for_tile(star_df, tile_df, proximity, Nsel, star_type):
    ...
    if star_type == "standards":
        ...
        raise NameError(
            f"star_type must be either 'standards' or 'guides'; currently {star_type}"
        )
...
def select_targets(
    ...
    while (len(targets) < Nsel) and (len(untiled_galaxies) > 0):
        ...
        # This is a dictionary of the priority of every target and then the number of things it clashes with, along with its index
        clash_and_priority_dict = dict(
            zip(
                untiled_galaxies,
                zip(priorities.loc[df.index].values, n_clashes, df.index),
            )
        )
        ...
        galaxies_in_order = sorted(
            clash_and_priority_dict.items(), key=itemgetter(1), reverse=True
        )
        ...
        if np.sum(n_clashes) > 0:
            ...
            isel_values.append(priorities[selected_target_index] + 1)

            # ... and remove the things this target clashes with from the list of targets in this tile
            untiled_galaxies = [
                u for u in untiled_galaxies if u not in things_it_clashes_with
            ]
        ...
        # Else if there aren't any clashes remaining...
            ...
            unclashing_targets_indices = [
                galaxies_in_order[r][1][-1] for r in range(N_to_append)
            ]

            targets.extend(unclashing_targets)
            isel_values.extend(priorities[unclashing_targets_indices])
    ...
    # Set up a boolean flag we can propogate if our tile doesn't have 19 objects in it
    tile_length_warning = len(tile_df) < Nsel
    ...
    if fill_spares_with_repeats:
        if len(tile_df) < Nsel:
            ...
            clashes = find_clashes(
                pd.concat((tile_df, already_tiled_in_FOV)),
                already_tiled_in_FOV,
                proximity,
            )
            n_clashes = clashes.sum(axis=0)
            if len(already_tiled_in_FOV) > 0 & (np.any(n_clashes == 0)):
                ...
                repeats_to_fill_hexabundles = np.random.choice(
                    unclashing_repeats, N_to_append, replace=False
                )
                tile_df = pd.concat(
                    (tile_df, already_tiled_in_FOV.iloc[repeats_to_fill_hexabundles])
                )
    ...
    if len(tile_df) < Nsel:
        ...
        if len(all_targets_df) > len(tile_df):
            ...
            targets_not_already_in_tile = all_targets_df[
                ~all_targets_df.index.isin(tile_df.index)
            ]
            clashes = find_clashes(
                targets_not_already_in_tile, tile_df, proximity=proximity
            )
            n_clashes = clashes.sum(axis=1)
    ...
    if np.sum(clashes) != 0:
        raise ValueError(
            "Our tile seems to clash with itself... This should never happen!"
        )

    if tile_length_warning:
        message = f"Can't select {Nsel} targets for this tile! Only have {len(tile_df)}"
        logger.warning(message)

    return tile_df, isel_values, tile_length_warning
...
def make_best_tile(
    ...
    if Hector_FOV_inner_radius < 0:
        raise ValueError(
            f"FoV Inner radius must be positive! Currently it's {Hector_FOV_inner_radius}"
        )
    if Hector_FOV_outer_radius < 0:
        raise ValueError(
            f"FoV Outer radius must be positive! Currently it's {Hector_FOV_outer_radius}"
        )
    if Hector_FOV_outer_radius < Hector_FOV_inner_radius:
        raise ValueError(
            f"Outer radius must be greater than inner radius! Currently O.R. is {Hector_FOV_outer_radius} and I.R. is {Hector_FOV_inner_radius}"
        )
    ...
    if tiling_type == "greedy":
        tile_RA, tile_Dec = get_best_tile_centre_greedy(
            df_targets.loc[untiled],
            outer_FOV_radius=Hector_FOV_outer_radius,
            inner_FoV_radius=Hector_FOV_inner_radius,
            n_xx_yy=n_xx_yy,
            priorities=priorities.loc[untiled],
        )
    elif tiling_type == "dengreedy":
        tile_RA, tile_Dec = get_best_tile_centre_dengreedy(
            df_targets,
            df_targets.loc[untiled],
            outer_FOV_radius=Hector_FOV_outer_radius,
            inner_FoV_radius=Hector_FOV_inner_radius,
    ...
    inner_targets = df_targets.loc[
        check_if_in_fov(
            df_targets,
            tile_RA,
            tile_Dec,
            outer_radius=Hector_FOV_outer_radius,
            inner_radius=Hector_FOV_inner_radius,
        ),
        :,
    ]
    inner_guides = df_guide_stars.loc[
        check_if_in_fov(
            df_guide_stars,
            tile_RA,
            tile_Dec,
    ...
    inner_standards = df_standard_stars.loc[
        check_if_in_fov(
            df_standard_stars,
            tile_RA,
            tile_Dec,
    ...
    tile_members, isel_values, tile_length_warning = select_targets(
        inner_targets,
        proximity,
        Nsel,
        priorities=priorities,
        selection_type=selection_type,
        fill_spares_with_repeats=fill_spares_with_repeats,
    )
    ...
    guide_stars_for_tile = select_stars_for_tile(
        inner_guides,
        tile_members,
        proximity=proximity,
        Nsel=Nsel_guides,
    ...
    standard_stars_for_tile = select_stars_for_tile(
        inner_standards,
        tile_members,
        proximity=proximity,
        Nsel=Nsel_standards,
...
def save_tile_outputs(
    ...
    if tile_length_warning:
        tile_out_name = f"{tile_out_name}.too_few_galaxies"
        guide_out_name = f"{guide_out_name}.too_few_galaxies"

    # Add in the MagnetX and MagnetY values, using the Hector plate radius from the constants file
    tile_df["MagnetX_noDC"] = (
        (tile_df["RA"] - tile_RA) * hector_constants.HECTOR_plate_radius * 1e3
    )
    tile_df["MagnetY_noDC"] = (
        (tile_df["DEC"] - tile_Dec) * hector_constants.HECTOR_plate_radius * 1e3
    )

    standard_stars_for_tile["MagnetX_noDC"] = (
        (standard_stars_for_tile["RA"] - tile_RA)
        * hector_constants.HECTOR_plate_radius
        * 1e3
    )
    standard_stars_for_tile["MagnetY_noDC"] = (
        (standard_stars_for_tile["DEC"] - tile_Dec)
        * hector_constants.HECTOR_plate_radius
        * 1e3
    )

    guide_stars_for_tile["MagnetX_noDC"] = (
        (guide_stars_for_tile["RA"] - tile_RA)
        * hector_constants.HECTOR_plate_radius
        * 1e3
    )
    guide_stars_for_tile["MagnetY_noDC"] = (
        (guide_stars_for_tile["DEC"] - tile_Dec)
        * hector_constants.HECTOR_plate_radius
        * 1e3
    )
    ...
    if plot:
        ...
        fig, ax = plot_tile(
            ...
            tile_RA,
            tile_Dec,
            tiling_parameters["Hector_FOV_outer_radius"],
            tiling_parameters["Hector_FOV_inner_radius"],
            tile_number,
            tiling_parameters["proximity"],
        )
...
def plot_survey_completeness_and_tile_positions(
    tile_positions,
    df_targets,
    tiling_parameters,
    fig=None,
    ...
    for tile_number in tiles:
        members = df_targets.loc[
            tiles[tile_number]
        ]  # This selects by index, NOT integer position.
        axs[0].scatter(members.RA, members.DEC)
        tile_footprint = plt.Circle(
            xy=(
                tile_RAs[tile_number - starting_tile_number],
                tile_DECs[tile_number - starting_tile_number],
            ),
            radius=tiling_parameters["Hector_FOV_outer_radius"],
            facecolor="None",
            edgecolor="k",
        )
    ...
    (
        completeness,
        completion_fraction_to_calculate,
        used_tiles_to_get_to_x,
        minimum_number_of_tiles_for_x,
        efficiency_xpc,
        efficiency,
    ) = calculate_completeness_stats(
        df_targets,
        tiling_parameters["N_targets_per_Hector_field"],
        completion_fraction_to_calculate=completion_fraction_to_calculate,
        verbose=verbose,
    )
    ...
    axs[1].plot(
        xx,
        tiling_parameters["N_targets_per_Hector_field"] / N_galaxy_observations * xx,
        c="k",
        linestyle="dashed",
    )
...
def _calc_completeness(df_targets):
    ...
    """
    completeness = np.cumsum(
        [
            np.sum((df_targets["Tile_number"] == i) & df_targets["COMPLETED"] == True)
            / len(df_targets)
            for i in np.unique(df_targets["Tile_number"])
        ]
    )
...
def calculate_completeness_stats(
    df_targets,
    N_targets_per_Hector_field,
    completion_fraction_to_calculate=0.95,
    verbose=True,
):
    """
    ...
    if (completion_fraction_to_calculate > 1) or (completion_fraction_to_calculate < 0):
        raise ValueError(
            f"completion_fraction_to_calculate must be between 0 and 1: currently {completion_fraction_to_calculate}"
        )
    ...
    minimum_number_of_tiles = np.ceil(
        N_galaxy_observations / N_targets_per_Hector_field
    )
    efficiency = minimum_number_of_tiles / used_tiles

    used_tiles_to_get_to_x = (
        np.where(completeness > completion_fraction_to_calculate)[0][0] + 1
    )
    minimum_number_of_tiles_for_x = np.ceil(
        completion_fraction_to_calculate
        * N_galaxy_observations
        / N_targets_per_Hector_field
    )
    ...
    if verbose:
        print(
            f"Efficiency for completion={completion_fraction_to_calculate}: {100*efficiency_xpc:.3f}%"
        )
...
def plot_tile(
    ...
    Args:
        ...
        mpl_style (optional, default='default'): A matplotlib style to use
    """
    plt.style.use(mpl_style)
    ...
    ax.scatter(
        tile_df.RA, tile_df.DEC, marker="x", c="k", s=30, label="All Targets", zorder=10
    )
    ax.scatter(
        catalogue_df.loc[top_target_mask, "RA"],
        catalogue_df.loc[top_target_mask, "DEC"],
        marker="x",
        c="r",
        s=50,
        label="Top Targets",
    ...
    ax.scatter(
        standards_df.RA, standards_df.DEC, marker="o", c="g", s=20, label="Standards"
    )
    ...
    circle = plt.Circle(
        (tile_RA, tile_Dec), tile_outer_radius, facecolor="None", edgecolor="r"
    )
    ax.add_artist(circle)
...
def save_tile_text_file(
    ...
    combined_stars_targets_df = pd.concat(
        (tile_df, standard_stars_for_tile), sort=True
    )[columns_in_order]
    ...
    combined_stars_targets_df.to_csv(
        f"{outfolder}/Tiles/{out_name}", sep=",", index=False, mode="a"
    )

    return 0


def save_guide_text_file(
    outfolder,
    out_name,
    guide_stars_for_tile,
    tile_RA,
    ...
    guide_stars_for_tile[guide_columns_in_order].to_csv(
        f"{outfolder}/Tiles/{out_name}", sep=" ", index=False, mode="a"
    )
