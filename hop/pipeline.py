...
from .hexabundle_allocation.problem_operations import (
    extract_data,
    file_arranging,
    hexabundle,
    offsets,
...
class HectorPipe:
    ...
    def __init__(
        ...
        if config_filename is None:
            assert (
                config_dictionary is not None
            ), "You must provide one of config_filename or config_dictionary"
            self.config = config_dictionary
        if config_dictionary is None:
            assert (
                config_filename is not None
            ), "You must provide one of config_filename or config_dictionary"
        ...
        subfolders_to_be_made = [
            "Logs",
            "Configuration",
            "Tiles",
            "Plots",
            "DistortionCorrected",
            "DistortionCorrected/Plots",
            "Allocation",
            "Allocation/tile_outputs",
            "Allocation/robot_outputs",
            "Fibres",
            "FinalOutputs",
        ]
        folders = misc_tools.create_output_directories(
            self.config["output_folder"], subfolders_to_be_made
        )
        ...
        self.DistortionCorrection_binary_location = Path(__file__).parent / Path(
            "distortion_correction/HectorTranslationSoftware/HectorConfigUtility/HectorConfigUtil"
        )
        ...
        self.Hector_distortion_file_location = Path(__file__).parent / Path(
            "distortion_correction/HectorTranslationSoftware/DataFiles/fit2-b-pos/HectorDistortion.sds"
        )
        self.Hector_linearity_file_location = Path(__file__).parent / Path(
            "distortion_correction/HectorTranslationSoftware/DataFiles/fit2-b-pos/HectorLinear.sds"
        )
        self.Hector_sky_fibre_location = Path(__file__).parent / Path(
            "distortion_correction/HectorTranslationSoftware/DataFiles/SkyFibres.csv"
        )

        if Profit_files_location is None:
            self.Profit_files_location = Path(__file__).parent / Path(
                "distortion_correction/HectorTranslationSoftware/DataFiles"
            )
        else:
        ...
        if not self.Hector_distortion_file_location.exists():
            raise FileNotFoundError(
                "The Hector distortion file HectorDistortion.sds seems to not exist"
            )
        os.environ[
            "HECTOR_DISTORTION"
        ] = self.Hector_distortion_file_location.as_posix()
        if not self.Hector_linearity_file_location.exists():
            raise FileNotFoundError(
                "The Hector linearity file HectorLinear.sds seems to not exist"
            )
        os.environ["HECTOR_LINEARITY"] = self.Hector_linearity_file_location.as_posix()

        self.ConfigurationCode_location = Path(__file__).parent / Path(
            "configuration/HECTOR_ClusterFieldsTest.R"
        )
        ...
        self.excel_files_for_allocation_location = Path(__file__).parent / Path(
            "hexabundle_allocation"
        )
        # Location of the P and Q offset file
        self.offsetFile = self.excel_files_for_allocation_location / Path(
            P_and_Q_offset_filename
        )
        # Location of the Fibre Slit Info file
        self.fibre_file = self.excel_files_for_allocation_location / Path(
            "Fibre_slitInfo_final.csv"
        )
    ...
    def make_header_dictionary(self):
        header_dictionary = {
            "#PROXIMITY": f"{self.config['proximity']} # tiling proximity value in arcseconds",
    ...
    def read_header_dictionary_from_file(self, filename):
        ...
        with open(filename, "r"):
            for line in self._read_file_get_header(filename)[0]:
    ...
    def load_input_catalogue(self):
        ...
        """
        self.df_targets = misc_tools._read_table(
            os.path.expanduser(self.config["final_catalogue_name"])
        )
        self.df_guide_stars = misc_tools._read_table(
            os.path.expanduser(self.config["guide_star_filename"])
        )
        self.df_standard_stars = misc_tools._read_table(
            os.path.expanduser(self.config["standard_star_filename"])
        )
    ...
    def _add_columns_to_catalogues(self):
        ...
        if "N_observations_to_complete" not in self.df_targets.columns:
            self.df_targets["N_observations_to_complete"] = 1

        self.df_targets["remaining_observations"] = self.df_targets[
            "N_observations_to_complete"
        ].copy()
        self.df_targets = self.df_targets.loc[
            self.df_targets["remaining_observations"] > 0
        ]
    ...
    #     self.df_targets = self.df_targets.rename(
    #         columns=dict(GAL_MAG_R="r_mag", CATID="ID")
    #     )
    #     self.df_guide_stars = self.df_guide_stars.rename(
    #         columns=dict(ROWID="ID", R_MAG_AUTO="r_mag")
    #     )
    #     self.df_standard_stars = self.df_standard_stars.rename(
    #         columns=dict(ROWID="ID", R_MAG_AUTO="r_mag")
    #     )
    ...
    def find_premade_tiles(self):
        ...
        try:
            self.df_targets = pd.read_csv(
                f"{self.tile_location}/in_progress_targets_dataframe.csv"
            )
        except FileNotFoundError:
            self.logger.info(
                "No 'in_progress_targets_dataframe.csv' file found, so no pre-made tiles will be loaded"
            )
            return 0

        pre_made_and_configured_files = np.sort(
            glob.glob(f"{self.configuration_location}/HECTORConfig_Hexa_*.txt")
        )
        pre_made_tiles = np.sort(
            glob.glob(os.path.expanduser(f"{self.tile_location}/tile_*.fld"))
        )

        starting_tile = 0
        for configuration_file, tile_file in zip(
            pre_made_and_configured_files, pre_made_tiles
        ):
    ...
    def _run_tiling_code(
        self,
        proximity,
        current_tile,
        use_galaxy_priorities,
        tile_out_fname=None,
        guide_out_fname=None,
    ):
        (
            df_targets,
            tile_df,
            guide_stars_for_tile,
            standard_stars_for_tile,
            tile_RA,
            tile_Dec,
            tile_length_warning,
        ) = tiling.make_best_tile(
            self.df_targets,
            self.df_guide_stars,
            self.df_standard_stars,
            proximity=proximity,
            tiling_parameters=self.config,
            tiling_type=self.config["tiling_type"],
            selection_type=self.config["allocation_type"],
            fill_spares_with_repeats=self.config["fill_spares_with_repeats"],
            use_galaxy_priorities=use_galaxy_priorities,
        )

        tiling.save_tile_outputs(
            f"{self.config['output_folder']}",
            self.df_targets,
            tile_df,
            guide_stars_for_tile,
            standard_stars_for_tile,
            ...
            plot=True,
            columns_in_order=self.config["columns_for_target_tile_saving"],
            guide_columns_in_order=self.config["columns_for_guide_tile_saving"],
            tile_out_name=tile_out_fname,
            guide_out_name=guide_out_fname,
            tile_length_warning=tile_length_warning,
        )
    ...
    def tile_field(
        ...
        self.logger.info(
            f"Robot Temperature: {robot_temperature} C, Obs Temperature: {obs_temperature} C. date={date}\n\n"
        )
        ...
        if (
            not self.have_targets_catalogue
            & self.have_standard_star_catalogue
            & self.have_guide_star_catalogue
        ):
            raise ValueError(
                "We must have a targets, guide star and standard star catalogue! Did you forget to load them?"
            )
        ...
        #     starting_tile_number = self.find_premade_tiles()
        ...
        for count in itertools.count():
            ...
            current_tile = count + starting_tile_number
            self.logger.info(f"Tile {current_tile}:")
            self.logger.info(f"\tRemaining Targets: {remaining_targets}")

            # Set up the tile filenames
            tile_out_fname = Path(
                f"{self.tile_location}/{label}_T{current_tile:03}.fld"
            )
            guide_tile_out_fname = Path(
                f"{self.tile_location}/{label}_GT{current_tile:03}.fld"
            )
            tile_out_fname_after_DC = Path(
                f"{self.distortion_corrected_tile_location}/DC_tile_{current_tile:03}.fld"
            )
            guide_tile_out_fname_after_DC = Path(
                f"{self.distortion_corrected_tile_location}/guide_DC_tile_{current_tile:03}.fld"
            )
            configuration_output_filename = Path(
                f"{self.configuration_location}/HECTORConfig_Hexa_{self.config['output_filename_stem']}_{current_tile:03}.txt"
            )
            configuration_guide_filename = Path(
                f"{self.configuration_location}/HECTORConfig_Guides_{self.config['output_filename_stem']}_{current_tile:03}.txt"
            )
            configuration_plot_filename = Path(
                f"{self.configuration_location}/HECTORConfig_Plot_{self.config['output_filename_stem']}_{current_tile:03}.pdf"
            )

            # filename for a plot which compares the tile before/after distortion correction
            DC_correction_comparison_plot_filename = Path(
                f"{self.distortion_corrected_plot_location}/Tile_{current_tile:03}_comparison.pdf"
            )
            ...
            (
                ...
                tile_RA,
                tile_Dec,
            ) = self._run_tiling_code(
                proximity=self.config["proximity"],
                current_tile=current_tile,
                use_galaxy_priorities=use_galaxy_priorities,
                tile_out_fname=tile_out_fname.name,
                guide_out_fname=guide_tile_out_fname.name,
            )
            ...
            if apply_distortion_correction:
                ...
                DC_corrected_output_file = self.apply_distortion_correction(
                    tile_out_fname,
                    guide_tile_out_fname,
                    tile_out_fname_after_DC,
                    guide_tile_out_fname_after_DC,
                    tile_RA,
                    tile_Dec,
                    guide_stars_for_tile,
                    plot_save_filename=DC_correction_comparison_plot_filename,
                    date=date,
                    robot_temp=robot_temperature,
                    obs_temp=obs_temperature,
                    label=f"{self.config['SourceCat']} Tile {count:03}",
                    plateID=plateID,
                    distortion_file=self.Hector_distortion_file_location,
                    linearity_file=self.Hector_linearity_file_location,
                    sky_fibre_file=self.Hector_sky_fibre_location,
                    profit_file_dir=self.Profit_files_location,
                    check_sky_fibres=check_sky_fibres,
                )
                self.logger.info(
                    f"\tDistortion-corrected tile saved at {tile_out_fname_after_DC}..."
                )
            ...
            if configure_tiles:
                ...
                for j in range(self.config["MAX_TRIES"]):
                    ...
                    return_code = self.run_configuration_code(
                        tile_file_for_configuration,
                        guide_tile_for_configuration,
                        configuration_output_filename,
                        configuration_guide_filename,
                        configuration_plot_filename,
                        config_timeout,
                    )
                ...
                if not configured:
                    raise ValueError(
                        f"Couldn't configure this tile after {self.config['MAX_TRIES']} attempts."
                    )

            # Update the header of the files from the config code
            if configure_tiles:
                self.header_dictionary = misc_tools.update_header(
                    configuration_output_filename, self.header_dictionary
                )
                self.header_dictionary = misc_tools.update_header(
                    configuration_guide_filename, self.header_dictionary
                )
            ...
            new_targets = (selected_targets_mask) & (
                self.df_targets["COMPLETED"] == False
            )
            # Find the targets which have been completed and are just filling out the numbers
            repeated_targets = (selected_targets_mask) & (
                self.df_targets["COMPLETED"] == True
            )
            ...
            self.df_targets.loc[
                new_targets
                & (self.df_targets.loc[new_targets, "remaining_observations"] == 0),
                "COMPLETED",
            ] = True
            ...
            self.tile_database = pd.concat(
                (self.tile_database, tile_df), ignore_index=True
            )
            # self.tile_database.set_index("tile_number", inplace=True)
            self.logger.info(
                f"\tTile contains {len(tile_df)} galaxies, of which {tile_df['Filler_Galaxy'].sum()} are repeats"
            )
            self.logger.info(f"FINISHED TILE {current_tile}\n\n")

            # Save the overall database in its current form
            self.df_targets.to_csv(
                f"{self.config['output_folder']}/Tiles/in_progress_targets_dataframe.csv"
            )

            self.tile_database.to_csv(
                f"{self.config['output_folder']}/Tiles/tiles_dataframe.csv"
            )

        self.tile_positions = np.array([self.best_tile_RAs, self.best_tile_Decs])

        if plot:
            fig, ax = tiling.plot_survey_completeness_and_tile_positions(
                self.tile_positions,
                self.df_targets,
                self.config,
                fig=None,
                ax=None,
                completion_fraction_to_calculate=0.95,
                verbose=True,
                starting_tile_number=starting_tile_number,
            )
            fig.savefig(
                f"{self.config['output_folder']}/Plots/{self.config['output_filename_stem']}_field_plot.pdf",
                bbox_inches="tight",
            )
            plt.close("all")
        ...
        self.df_targets.to_csv(
            f"{self.config['output_folder']}/Tiles/completed_targets_dataframe.csv"
        )
    ...
    def apply_distortion_correction(
        ...
        obs_temp=12,
        label="test_label",
        plateID="test_plateID",
        distortion_file="",
        linearity_file="",
        sky_fibre_file="",
        profit_file_dir="",
        check_sky_fibres=True,
        turn_off_linearity=False,
        rot_matrix="",
        apply_PM_corr=True,
        debug_levels=None,
    ):
        ...
        DC_bash_code = [
            f"{self.DistortionCorrection_binary_location}",
            f"{tile_out_fname}",
            f"{guide_tile_out_fname}",
            f"{tile_out_fname_after_DC}",
            f"{label}",
            f"{plateID}",
            f"{date}",
            f"{robot_temp}",
        ...
        process = subprocess.Popen(
            DC_bash_code, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        ...
        guide_rows = DC_corrected_output_file.ID.isin(
            guide_stars_for_tile.ID.astype(str)
        )
        ...
        if plot_save_filename is not None:
            fig, ax = plotting_tools.plot_distortion_correction_before_after(
                tile_out_fname_after_DC, title_text=f"{tile_out_fname_after_DC}"
            )
    ...
    def apply_config_code_to_tile(self, tile_number, config_timeout=None):
        tile_file_for_configuration = Path(
            f"{self.distortion_corrected_tile_location}/DC_tile_{tile_number:03}.fld"
        )
        guide_tile_for_configuration = Path(
            f"{self.distortion_corrected_tile_location}/guide_DC_tile_{tile_number:03}.fld"
        )
        configuration_output_filename = Path(
            f"{self.configuration_location}/HECTORConfig_Hexa_{self.config['output_filename_stem']}_{tile_number:03}.txt"
        )
        configuration_guide_filename = Path(
            f"{self.configuration_location}/HECTORConfig_Guides_{self.config['output_filename_stem']}_{tile_number:03}.txt"
        )
        configuration_plot_filename = Path(
            f"{self.configuration_location}/HECTORConfig_Plot_{self.config['output_filename_stem']}_{tile_number:03}.pdf"
        )
    ...
    def run_configuration_code(
        ...
        Configuration_bash_code = [
            "Rscript",
            self.ConfigurationCode_location,
            tile_file_for_configuration,
            guide_tile_for_configuration,
            configuration_output_filename,
            configuration_guide_filename,
    ...
    def make_output_file_for_Tony(self, tile_file, robot_file):
        ...
        outputTileFile = Path(
            f"{self.final_tiles_correct_format_location}/Tile_FinalFormat_{filename_stem}.csv"
        )
        outputRobotFile = Path(
            f"{self.final_tiles_correct_format_location}/Robot_FinalFormat_{filename_stem}.csv"
        )
        ...
        tile_header_lines, tile_header_linenumbers = self._read_file_get_header(
            tile_file
        )
        ...
        # Replace NAs with -99
        output_tile_contents.loc[:, integer_columns] = (
            output_tile_contents.loc[:, integer_columns].fillna(-99).astype(int)
        )
        # Now change the types of these columns to be integers
        output_tile_contents = output_tile_contents.astype(
            dict(zip(integer_columns, ["int64"] * len(integer_columns)))
        )
        ...
        output_tile_contents["ID"] = output_tile_contents["ID"].apply(
            _fix_integer_value
        )

        self._write_file_with_header(
            outfilename=outputTileFile,
            header=tile_header_lines,
            dataframe=output_tile_contents.loc[:, columns],
        )

        # Now do the robot file
        robot_header_lines, robot_header_linenumbers = self._read_file_get_header(
            robot_file
        )

        output_robot_contents = pd.read_csv(robot_file, comment="#")
        self._write_file_with_header(
            outfilename=outputRobotFile,
            header=robot_header_lines,
            dataframe=output_robot_contents,
        )

    def allocate_hexabundles_for_single_tile(
        self,
        fileNameGuides,
        fileNameHexa,
        robot_temperature,
        ...
        df_guideFile, guideFileList = file_arranging.arrange_guidesFile(
            fileNameHexa, fileNameGuides, guide_outputFile
        )

        # Adding guides cluster txt file to hexa cluster txt file
        df_combined = file_arranging.merge_hexaAndGuides(
            fileNameHexa, df_guideFile, plate_file
        )

        # extracting all the magnets and making a list of them from the plate_file
        all_magnets = extract_data.create_list_of_all_magnets_from_file(
            extract_data.get_file(plate_file), guideFileList
        )
        ...
        all_magnets = offsets.magnetPair_radialPositionOffset_circularAnnulus(
            offset_circularAnnulus, all_magnets
        )
        ...
        (
            self.galaxyIDrecord,
            MagnetDict,
        ) = hexabundle.overall_hexabundle_size_allocation_operation_version3_largerBundlePriority(
            all_magnets,
            self.galaxyIDrecord,
            mu_1re_cutoff,
            self.config["output_filename_stem"],
            flagsFile,
        )
        ...
        fully_blocked_magnets = (
            conflicts.functions.create_list_of_fully_blocked_magnets(conflicted_magnets)
        )

        fully_blocked_magnets_dictionary = (
            conflicts.functions.blocking_magnets_for_fully_blocked_magnets(
                conflicted_magnets
            )
        )
        ...
        conflicts.blocked_magnet.print_fully_blocked_magnets(
            fully_blocked_magnets, conflictsRecord, fileNameHexa
        )

        # record the magnet and tile list in unresolvable conflicts file
        conflictFile = (
            f"{self.allocation_files_location_base}/unresolvable_conflicts.txt"
        )
        ...
        if plot:
            ...
            hexabundle_figureFile = (
                f"{self.plot_location}/hexabundlePlot_{fileNameHexa_stem}.pdf"
            )
            fig_hexa, fig_robot = plots.draw_all_magnets(
                all_magnets,
                self.config["output_filename_stem"],
                fileNameHexa,
                robot_figureFile,
        ...
        # Output file 3
        robotFile = (
            f"{self.allocation_files_location_robot}/Robot_{fileNameHexa_stem}.txt"
        )

        ## DUMMY VALUES TO BE REMOVED ##
        # creating robotFile array and storing it in robot file
        positioning_array, robotFilearray = file_arranging.create_robotFileArray(
            label_string,
            positioning_array,
            robotFile,
            newrow,
        ...
        (
            positioning_array,
            positioning_array_circular,
        ) = file_arranging.positioningArray_adjust_and_mergetoFile(
            positioning_array, plate_file, outputFile, newrow, newrow_circular
        )
        ...
        output_fibreAAOmega = (
            f"{self.allocation_files_location_tiles}/Hector_fibres_AAOmega.txt"
        )
        output_fibreSpector = (
            f"{self.allocation_files_location_tiles}/Hector_fibres_Spector.txt"
        )

        output_hexabundle_coordData = (
            f"{self.allocation_files_location_tiles}/Fibre_coordData_"
        )
        ...
        fibres.create_fibreSlit_info_file(
            fileNameHexa, self.fibre_file, output_fibreSlitInfo
        )

        # create the fibre spectrograph files for each of AAOmega and Spector
        new_arrayAAOmega, new_arraySpector = fibres.extract_fibreInfo(
            self.fibre_file, output_fibreAAOmega, output_fibreSpector
        )
        ...
        fibreFigure_AAOmega = (
            f"{self.plot_location}/fibre_slitletAAOmega_{fileNameHexa_stem}.pdf"
        )
        # Output file- figure 4 fibres
        fibreFigure_Spector = (
            f"{self.plot_location}/fibre_slitletSpector_{fileNameHexa_stem}.pdf"
        )
        # create figure of slitlets
        fibres.create_slitletFigure(
            new_arrayAAOmega, new_arraySpector, fibreFigure_AAOmega, fibreFigure_Spector
        )

        # Output file- figure 5 fibres
        skyFibre_AAOmegaFigure = (
            f"{self.plot_location}/skyfibre_slitletAAOmega_{fileNameHexa_stem}.pdf"
        )
        # Output file- figure 6 fibres
        skyFibre_SpectorFigure = (
            f"{self.plot_location}/skyfibre_slitletSpector_{fileNameHexa_stem}.pdf"
        )
        # create figure of magnified sky fibre positioning in slitlets
        fibres.create_skyFibreSlitlet_figure(
            fileNameHexa=fileNameHexa,
            new_arrayAAOmega=new_arrayAAOmega,
            new_arraySpector=new_arraySpector,
            skyFibre_AAOmegaFigure=skyFibre_AAOmegaFigure,
            skyFibre_SpectorFigure=skyFibre_SpectorFigure,
        )
    ...
    def allocate_hexabundles_for_all_tiles(self):
        all_configured_tiles = np.sort(
            glob.glob(f"{self.configuration_location}/HECTORConfig_Hexa_*.txt")
        )

        tile_numbers = [
            int(t.split("_")[-1].split(".")[0]) for t in all_configured_tiles
        ]
    ...
    def run_target_selection(self, random_generator, plot=False, save=False):
        full_catalogue_table = HectorSim.load_table(self.config)
        HectorSim_input_parameters = [
            "BoundaryType",
            "zlimit",
            "MstarMin",
            "MstarMax",
        ...
        HectorSim_args["random_generator"] = random_generator
        self.target_selection = HectorSim.HectorSim(**HectorSim_args)

        if save:
            final_catalogue_path = Path(self.config["final_catalogue_name"])
            P.save_dataframe_as_FITS_table(
                self.target_selection.selection_function_sparsely_sampled,
                final_catalogue_path.expanduser().as_posix(),
            )
    ...
    def _make_target_selection_plot(self):
        plt.style.use(
            (Path(__file__).parent / "target_selection/HectorSim.mplstyle").as_posix()
        )
    ...
    def show_skyfibreChanges_and_magnetCountPerAnnuli(
        self,
        tile_fname_1=None,
        tile_fname_2=None,
        tile_1_guide=None,
        tile_2_guide=None,
        tile_batch=None,
        tileBatch_count=None,
        tileBatch_skyFibreChange="OFF",
    ):
        ...
        if (tile_fname_1 != None) and (tile_fname_2 != None):
            ...
            fibres.createskyfibreChanges_plot(
                self,
                tile_fname_1,
                tile_fname_2,
                subplateSkyfibre_figureFile_tile1,
                subplateSkyfibre_figureFile_tile2,
                subplateSkyfibre_figureFile,
            )
            ...
            for i in range(tileBatch_count + 1):
                for j in range(i, tileBatch_count + 1):
                    if i != j:
                        annuliCount_batch = fibres.check_magnetCount_perAnnulus(
                            self, i, j, annuliCount_batch, annuli_count_magnetCasing
                        )
                        ...
                        if tileBatch_skyFibreChange == "ON":
                            ...
                            pistonChange_count = (
                                fibres.skyfibreChanges_pistonChange_count(self, i, j)
                            )
                            pistonChange_countBatch += [pistonChange_count]

            # create histogram plot
            fibres.plotHist_annuliCount_batch(
                self, annuliCount_batch, tile_batch, tileBatch_count
            )

            if tileBatch_skyFibreChange == "ON":
                fibres.plotHist_pistonChange_count_batch(
                    self, pistonChange_countBatch, tile_batch, tileBatch_count
                )
